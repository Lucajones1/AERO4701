...
% Copper Coil
B3 = [(coords_coil(1,1)+coords_coil(2,1))/2,10];
...
mi_savebitmap(['FEMMGeometry','.eps']);
...
% Integrating over the contour, using the Maxwell Stress Tensor, to give
% the resulting Force generated by the Magnetorquer
Fvec = mo_lineintegral(3);
...
mo_zoomnatural();
mo_showdensityplot(1,0,round(1.1*maxB,4),0,'mag');
mo_savebitmap(['FEMMContour','.eps']);
